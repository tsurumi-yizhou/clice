...
索引目前的定位是服务跨文件导航——一份为"符号在哪里"而设计的数据。但沿着预计算的思路走到底，会看到一个更大的可能性：当文件没有被修改时，整个文件都是不可变内容，理论上不需要构建任何 AST，从磁盘索引就能直接响应请求。

设想用户打开一个干净的文件，只是浏览、跳转、查引用。这些信息后台索引早已算过一遍，为一次只读访问重新构建完整 AST 是纯粹的浪费。只读模式下，服务器读索引即可响应——无 AST、无 PCH、近乎零启动成本。用户开始编辑时才升级到实时编译会话；停止编辑且内容回到已索引的状态后，又可以降回索引响应。响应的来源在"磁盘索引 → 预计算 preamble 加实时尾部 → 完整实时 AST"之间随内容的可变程度滑动，对客户端完全透明。

前提是索引的 schema 从"导航所需"扩展到"响应 LSP 请求所必要的全部信息"——不多存，也不少存。这远比想象的紧凑：AST 是为编译设计的，而响应一次 hover、渲染一屏 semantic tokens 所需要的只是提炼后的少量行。远程与 Web 环境是这个模式的直接受益者：静态托管一份索引，就能提供代码浏览级别的完整语言服务。

这与微软的 LSIF、Sourcegraph 的 SCIP 思路同源——都是预计算的代码智能快照——但定位不同。LSIF/SCIP 是编译无关的通用交换格式，是纯静态的终点；clice 的索引为 C++ 特化（编译上下文区分、内容寻址去重、preamble 分层），而且不是终点，是连续谱的一端——静态快照与实时 AST 之间可以合并与切换，这是通用格式做不到的。

统一语义层的重构已经为这条路铺了一段：功能实现与数据来源解耦了。semantic tokens 如今是"在表上渲染"而不是"在 AST 上现算"，而这张表来自实时构建还是磁盘反序列化，对渲染逻辑是透明的。schema 具体要为哪些功能存哪些行、只读与实时会话之间的升降级如何判定，这些留到第一个真实消费者落地时再固化。
