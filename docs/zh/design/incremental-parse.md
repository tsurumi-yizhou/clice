...
Preamble 分离带来的不只是编译加速，还有一个容易被忽视的性质：preamble 一旦编译成 PCH，它的内容在 PCH 的整个生命周期内不再变化。而代价是，主文件的编译加载 PCH 后就看不到这个区域了——preamble 区的指令、token 流都被 PCH 吞掉，对活 AST 而言是个黑盒。如果每个功能都在请求时各自想办法绕过这个黑盒，代码会碎成一地特判。

clice 的做法是抓住 PCH 构建的那一刻——preamble 唯一一次拥有完整活 AST 的时机——把这个区域的功能结果一次性算好：符号索引、document links、非活跃区域、未闭合条件栈，全部序列化进与 PCH 配对的状态 blob。之后的每次请求，功能层把预计算结果与主文件的实时结果合并：document links 是两段拼接，跳转和引用是 overlay 查询。预计算结果的有效期与 PCH 相同，不需要任何独立的失效逻辑——preamble 变了，PCH 和 blob 一起重建。

这背后是一条值得明确的分界：**可变的内容由活 AST 响应，不可变的内容由预计算结果响应，两者按功能各自定义合并方式**。它也回答了一个曾经纠结过的设计问题——要不要把 preamble 状态传给主文件编译、让它参与语义层的构建？答案是不要：能序列化传递的只有行级的结果数据，永远拼不回节点级的 AST 结构；行级数据的正确用法就是预计算加合并，不需要绕经语义层。新功能想覆盖 preamble 区域时（比如 preamble 区的 semantic tokens），做法也相同：在 PCH 构建侧算出结果行加入 blob，再定义与主文件结果的合并方式。

沿这条分界还能走得更远。bound 不必永远固定在 include 块的结尾：对上万行的巨型源文件，或触发 Clang 病态行为（如巨型常量数组）的代码，可以把缓存边界推进到文件更深处——由用户指定或启发式选择。前缀包含真实代码也不破坏该机制，因为前缀的功能结果照样在 PCH 构建时算好；前缀越大，增量编译越便宜，预计算的红利越大。
